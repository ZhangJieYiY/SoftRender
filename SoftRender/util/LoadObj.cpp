...
Object* LoadObj::loadFromFile(const string& vname) {//读取obj 文件内容生成绘制用物体对象的方法
	Object * lo; //指向生成的绘制用物体对象的指针
	vector<float> alv;
	...
	double* vPosData = new double[vCount * 3]; //创建顶点数据数组
	double* vUVData = new double[vCount * 2]; //创建顶点数据数组
	double* vNormalData = new double[vCount * 3]; //创建顶点数据数组
	...
	for (int i = 0; i<vCount; i++) { //遍历每个顶点
		...
		vNormalData[i * 3 + 0] = alnResult[i * 3 + 0]; //将法向量x 分量转存到顶点数据数组中
		vNormalData[i * 3 + 1] = alnResult[i * 3 + 1]; //将法向量y 分量转存到顶点数据数组中
		vNormalData[i * 3 + 2] = alnResult[i * 3 + 2]; //将法向量z 分量转存到顶点数据数组中
	}
	lo = new Object(vPosData, vUVData, vNormalData, vCount);//创建绘制用物体对象
